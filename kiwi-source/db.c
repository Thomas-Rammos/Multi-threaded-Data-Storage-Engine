...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    pthread_mutex_init(&(self->readwrite_mutex), NULL); //initialize the mutex
    pthread_cond_init(&(self->readwrite_cond), NULL); //initialize the condition
    self->num_readers = 0;
...
void db_close(DB *self)
{
	...
	pthread_mutex_destroy(&(self->readwrite_mutex)); //destroy mutex at close
    pthread_cond_destroy(&(self->readwrite_cond)); //destroy cond at close
    free(self);
}
...
void readers_lock(DB* self)
{
    ...
    while (self->writer_active) {
        pthread_cond_wait(&self->readwrite_cond, &self->readwrite_mutex); // wait for the condition to be signal
    }
